...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'tube-dist/video-medicaltalk-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'tube-dist/video-medicaltalk-tube/browser'), 'index.html')
    )
    .toString();
